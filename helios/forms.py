...
class ElectionForm(forms.Form):
  short_name = forms.SlugField(max_length=40, widget=forms.TextInput(attrs={'size':60, 'placeholder':'no spaces, will be part of the URL for your election, e.g. my-club-2010',}))
#  help_text='no spaces, will be part of the URL for your election, e.g. my-club-2010',

  name = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'size':60, 'placeholder':'the pretty name for your election, e.g. My Club 2010 Election',}))
  ...
  election_type = forms.ChoiceField(widget=forms.RadioSelect, label="Election type", choices = Election.ELECTION_TYPES)
  ...
  # times
  voting_starts_at = SplitDateTimeField(help_text = 'EAT date and time when voting begins',
                                   widget=SplitSelectDateTimeWidget, required=False)
  voting_ends_at = SplitDateTimeField(help_text = 'EAT date and time when voting ends',
                                   widget=SplitSelectDateTimeWidget, required=False)
...
class EmailVotersForm(forms.Form):
  ...
  # ian text edits
class TextVotersForm(forms.Form):
  subject = forms.CharField(max_length=80)
  body = forms.CharField(max_length=4000, widget=forms.Textarea)
  send_to = forms.ChoiceField(label="Send To", initial="all", choices= [('all', 'all voters'), ('voted', 'voters who have cast a ballot'), ('not-voted', 'voters who have not yet cast a ballot')])
